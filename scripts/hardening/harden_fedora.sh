#!/bin/bash
#
# Hardening script for Fedora systems
# Tested on: Fedora 38-42
#
...
# --- HELPER FUNCTIONS ---
# Global password variables (set by prompt_password)
_PROMPTED_PASS=""
...
# Only do this if you're sure you don't need key-based auth
...
dnf remove -y openssh-server || echo "openssh-server not installed or already removed"
...
# Save Rules (multiple methods for compatibility)
...
MAIL_SCRIPT="$SCRIPT_DIR/../services/mail/mail_hardener_fedora.sh"
...
if [[ -f "$MAIL_SCRIPT" ]]; then
    echo "Running mail hardening script"
    bash "$MAIL_SCRIPT" >> "$LOG_FILE" 2>&1 || echo "Mail hardening had errors"
else
    echo "Mail hardening script not found, skipping"
fi
...
# Backup mysql/mariadb configs (Fedora uses mariadb typically)
if [[ -d /etc/my.cnf.d ]]; then
    cp -r /etc/my.cnf.d "$BACKUP_DIR/mysql_$TIMESTAMP" 2>/dev/null || true
fi
