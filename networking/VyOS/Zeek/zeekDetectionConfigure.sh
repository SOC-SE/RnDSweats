...
install_bzar() {
    ...
    if ! command -v zkg &> /dev/null; then
        ...
        if command -v pip3 &> /dev/null; then
            ...
            log_info "Install manually: pip install zkg"
            log_info "Then run: zkg install zeek/mitre-attack/bzar"
            return 1
        fi
    fi
    
    # Re-check if zkg is now available
    if ! command -v zkg &> /dev/null; then
        log_warning "zkg still not available after install attempt"
        return 1
    fi
    ...
    if zkg install zeek/mitre-attack/bzar --force 2>/dev/null; then
...
create_local_zeek() {
    ...
    if [[ "$BZAR_INSTALLED" == true ]]; then
        ...
        bzar_load="# @load packages/bzar  # Uncomment after installing: zkg install zeek/mitre-attack/bzar"
    fi
...
# Enable JA3/JA4 fingerprinting (if available)
@load policy/protocols/ssl/ja3
# @load policy/protocols/ssl/ja4  # Uncomment if Zeek has JA4 support
...
print_summary() {
    ...
    echo "  ✓ TLS Fingerprinting (JA4/JA3) - 120+ signatures"
...
# Step 1: Detect Zeek
log_step "1/7" "Detecting Zeek Installation"
if [[ -z "$ZEEK_DIR" ]]; then
    if ! detect_zeek; then
...
# Step 2: Install BZAR
log_step "2/7" "MITRE BZAR Package"
...
# Step 3: Install TLS Fingerprinting
log_step "3/7" "TLS Fingerprinting Framework"
install_tls_fingerprinting

# Step 4: Generate Fingerprints
log_step "4/7" "Fingerprint Database"
generate_fingerprints

# Step 5: Install AD Attacks
log_step "5/7" "AD Attack Detection"
install_ad_attacks

# Step 6: Collect Whitelists (optional, interactive)
log_step "6/7" "Whitelist Configuration (Optional)"
if [[ "$NON_INTERACTIVE" == true ]]; then
