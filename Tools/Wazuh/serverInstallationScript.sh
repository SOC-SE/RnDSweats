...
if ! docker --version; then
  echo "Hey, you need docker for this. I'll try and install it."
  dnf install -y docker || exit $?
fi

if ! $"docker compose" --version; then
  echo "Hey, you need docker compose for this. I'll try and install it."
  dnf install -y docker compose || exit $?
fi

# Don't install on a full stomach. You'll get cramps
if [ -n "$(ls)" ]; then
    echo "This directory isn't empty."
    echo 'Enter the name of a directory to install into ("." for the current directory anyway)'
    read -p '> ' INSTALL_DIR
...
function genPassword() {
    cat /dev/urandom | tr -cd '[:alnum:].*+_\-<>' | head -c64
}
...
if [ -f config/wazuh_indexer_ssl_certs/root-ca.pem ]; then
    echo 'Hey, it looks like you already have a CA generated!'
    echo "Do you want to just keep what you've got? (Y/n)"
    read PROMPT
else
    PROMPT="n"
fi

test "$PROMPT" == "n" && docker compose -f generate-indexer-certs.yml run --rm generator || exit $?

#endregion Generate Certificates
